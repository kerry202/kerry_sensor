...
import com.sensor.test.kerry.shimmer.Shimmer;
import com.sensor.test.kerry.shimmer.ShimmerTextView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (wtd) {
            ...
            case Sensor.TYPE_GRAVITY:
                ...
                accelSensor = sensorManager.getDefaultSensor(wtd);
                sensorManager.registerListener(threeParamListener, accelSensor, sensorManager.SENSOR_DELAY_FASTEST);
                yMax = 100;
                sensor_id = wtd;
                break;
...
//                giveAverage(sum);//将当前测量的结果写入buffer，然后定期求buffer里面的平均值，并显示
                Log.e("   SensorEventListener ", " x  " + event.values[0] + " Y " + event.values[1] + " Z " + event.values[2] + " timestamp "+event.timestamp);
